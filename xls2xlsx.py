...
class XLS2XLSX:
    ...
    def __init__(self, f, dirname='.'):
        ...
        try:
            self.book = xlrd.open_workbook(
                file_contents=self.contents, formatting_info=True)
    ...
    def xls_style_to_xlsx(self, xf_ndx):
        ...
        if xf_ndx < len(self.book.xf_list):
            ...
            # FIXME: 0=Any, 1=Roman, 2=Sans, 3=monospace, 4=Script, 5=Old English/Franktur
            family = xls_font.family
            font.name = xls_font.name
            font.sz = self.xls_height_to_xlsx(
                xls_font.height)    # A twip = 1/20 of a pt
            ...
            fill_background_color = self.xls_color_to_xlsx(
                xf.background.background_colour_index)
            fill_pattern_color = self.xls_color_to_xlsx(
            ...
            horizontal = {0: 'general', 1: 'left', 2: 'center', 3: 'right',
        ...
        for sheet in self.book.sheets():
            ...
            for col_ndx, info in sheet.colinfo_map.items():
                ...
                if info.hidden:
                    ...
                    ws.column_dimensions[col].width = self.xls_width_to_xlsx(
                        info.width)
            for row_ndx, info in sheet.rowinfo_map.items():
                ...
                if info.hidden:
                    ...
                    ws.row_dimensions[row].height = self.xls_height_to_xlsx(
                        info.height)
